...
async fn main() {
    println!("linux http server starting");
...
async fn handle_request(req: Request<Body>) -> Result<Response<Body>, Infallible> {
    ...
    let response = match (req.method(), req.uri().path()) {
        (&Method::GET, "/health") => json_response(
            StatusCode::OK,
            r#"{"status":"ok","runtime":"linux"}"#.to_string(),
        ),
    ...
    match result {
        Ok(Ok(resp)) => {
            ...
            json_response(
                StatusCode::OK,
                format!(
                    r#"{{"status":"ok","runtime":"linux","upstream_status":{}}}"#,
                    status.as_u16()
                ),
            )
        }
        ...
        Ok(Err(e)) => {
            ...
            json_response(
                ...
                format!(
                    r#"{{"status":"error","runtime":"linux","message":"{}"}}"#,
        ...
        Err(_) => {
            ...
            json_response(
                ...
                r#"{"status":"timeout","runtime":"linux"}"#.to_string(),
            )
        }
    }
}
